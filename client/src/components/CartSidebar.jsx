        ...
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col justify-between border-l border-stone-200">
          ...
          <div className="p-6 border-b border-stone-100 flex items-center justify-between bg-stone-50/70">
            ...
            <button
              onClick={onClose}
              type="button"
              className="p-2 rounded-md text-stone-400 hover:text-stone-700 hover:bg-stone-100 transition-colors"
            >
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {items.length === 0 ? (
              <div className="text-center py-20 text-stone-400 space-y-3">
                ...
                <p className="text-base font-serif text-stone-700">Your bag is empty</p>
                <p className="text-xs text-stone-500 max-w-xs mx-auto">
                  Explore our artisanal fragrance collection and find your signature scent.
                </p>
              </div>
            ) : (
              items.map((item) => (
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <div className="flex items-center border border-stone-200 rounded-md bg-white overflow-hidden shadow-2xs">
                        <button
                          ...
                          className="px-2.5 py-0.5 text-stone-600 hover:bg-stone-100 text-xs transition-colors"
                          type="button"
                        >
                          -
                        </button>
          ...
          {items.length > 0 && (
            <div className="p-6 border-t border-stone-200 bg-stone-50/80 space-y-4">
              ...
              <form onSubmit={handleApplyPromo} className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-stone-900 text-white rounded-md text-xs uppercase font-medium tracking-wider hover:bg-amber-900 transition-colors"
                >
                  Apply
                </button>
              </form>
              ...
              <button
                onClick={onCheckout}
                type="button"
                className="w-full py-3.5 px-4 bg-amber-800 hover:bg-amber-900 text-white font-medium text-xs uppercase tracking-widest rounded-md transition-all shadow-md active:scale-98"
              >
                Proceed to Checkout
              </button>
            </div>
